...
- **docs:** `docs/systems/subject_monitor.md` 补充跳日规则与 HUD 隐藏说明。
...
- **ui:** 采集数据日志新增异化进度条直观显示当前异化百分��；保留淡青高亮样式（`pages/subject_monitor.js`/`index.css`）。
- **docs:** `docs/systems/subject_monitor.md` 更新采集日志高亮说明。
...
- 时间与��据：时间系统抽成 `modules/time.js` 并暴露订阅接口；实验体与日常状态池移到 `data/subjects.js`。
