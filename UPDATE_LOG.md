...
## 2025-11-28 16:30 — 战斗系统全面重构与完善
  ...
  - **feat(combat):** 实现护盾系统，伤害优先扣除护盾，剩余伤害扣除 HP。
  - **feat(combat):** 实现技能冷却(CD)系统，强力技能使用后进入冷却，CD 每回合递减。
  - **feat(combat):** 实现眩晕机制，被眩晕单位跳过行动，回合开始时自动解除。
  - **feat(combat):** 完善所有技能的特殊效果：
    - 条件额外伤害（对特定属性 +X 伤害）
    - 护盾破坏效果（溶断、暗焰刺、静默落雷等）
    - 技能复制效果（回声视野、口令复述）
    - 状态交换（相位移）
    - 概率触发（递归喙击 50% 额外伤害）
    - 反击/反伤效果（静电罩、黑烟掩护、反击姿态）
  - **feat(combat):** 添加战斗物品使用系统，支持治疗类和 Buff 类物品（纳米修补剂、过载注射）。
  - **ui(combat):** 战斗界面显示护盾数值（在 HP 旁显示 `+XX🛡️`）。
  - **ui(combat):** 新增 Buff/DOT 状态标签显示，蓝色标签显示增益效果，红色标签显示持续伤害，均显示剩余回合数。
