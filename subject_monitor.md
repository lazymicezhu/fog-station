...
- 日常状态池：每天为每个实验体分配“日常状态”（白/橙/红），颜色概率 70%/20%/10%，事件池分别 20/10/5 条描述，单色用完后再轮回不重复；日切/跳日时重新分配并写入日志，当前实验体信息备注显示“今日状态”。
...
- 日常状态池：每天为每个实验体分配“日常状态”（白/橙/红），颜色概率 70%/20%/10%，事件池分别 20/10/5 条描述，单色用完后再轮回不重复；游戏开始、日切、跳日时都会分配并写入日志，当前实验体信息备注显示“今日状态”，颜色用文字颜色标识。
