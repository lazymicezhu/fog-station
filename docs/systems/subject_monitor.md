...
- 日常状态池：每天为每个实验体分配“日常状态”（白/橙/红），颜色概率 70%/20%/10%，事件池分别 20/10/5 条描述，单色用完后再轮回不重复；日切/跳日时重新分配并写入日志，信息卡只显示当日状态文本（无“今日状态”前缀），颜色随状态而变（白/橙/红）。
...
- 日常状态池：每天为每个实验体分配“日常状态”（白/橙/红），颜色概率 70%/20%/10%，事件池分别 20/10/5 条描述，单色用完后再轮回不重复；游戏开始、日切、跳日时都会分配并写入日志，信息卡只显示当日状态文本并随状态染色（白/橙/红）。
...
- 响应按钮：`采集数据` 写备注日志；`刷新生命体征` 仅随机生命体征并轻触发轮廓动画。信息卡只显示当日状态，颜色与状态一致。
